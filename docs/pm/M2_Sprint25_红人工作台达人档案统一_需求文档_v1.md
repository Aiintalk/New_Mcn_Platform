...
1. 生成流程第一步增加“选择红人”必填项。
2. 选择项来自正式红人列表，提交时传递 `kol_id`。
3. 红人未选择时，不允许开始生成。
4. 选择红人后展示红人姓名、账号名称和当前档案完整度，帮助运营避免选错。
5. 生成过程和历史报告都保留红人关联，不得使用姓名或昵称反向猜测关联。

#### 数据要求

- `persona_reports` 需增加可追溯的 `kol_id` 外键。
- 历史报告不强制回填 `kol_id`，不做姓名批量匹配。
- 新报告必须关联有效、未删除的红人。
...
2. 人工修改一个事实字段，再生成新报告，验证人工字段未被覆盖。
3. 对已有人格档案和内容规划的红人生成新报告，验证两个字段可分别选择覆盖或保留。
4. 在红人管理和旧素材库查看同一红人，验证无并行编辑入口且内容一致。
5. 从红人工作台进入千川仿写、价值观仿写、直播仿写和复盘，验证七字段上下文均可用。
...
| 人格工具绑定达人 | 生成前强制选择具体红人，并展示账号和档案完整度 |
...
| 人格工具绑定达人 | 1440×900 | [查看截图](./prototypes/artifacts/M2_Sprint25_人格工具绑定达人_1440x900.png) |
