...
> 2026-08-03 Sprint25 达人档案统一（分支 `feature/kol-persona-profile-unification`，基线 `5264628d`）：开发实现、相关与邻接后端 841 项、迁移专项 1 项、前端 489 项、类型检查、生产构建、迁移全链重复执行、6 张双分辨率页面证据和独立只读技术复审 `APPROVED` 均完成。正式 `kols.id` 已贯通当前运营入驻资料、生成、报告、产出、历史和七字段档案；非空定位字段默认保留并逐项确认，五项事实只按报告原文补空；红人管理和旧素材库已收口；生成中删除、部分流中断、同步失败提示和历史覆盖恢复边界已通过失败测试闭环。当前状态仅为**可供产品经理验收**，未合并、未部署、未执行生产迁移、未上线。详见 `docs/pm/M2_Sprint25_红人工作台达人档案统一_开发验收报告_v1.md`。
...
> 最后更新：2026-07-31（**PR #35 `feat(admin-ai): 自定义 AI 厂商支持` 合并 main**（merge `5997b24b`）：ServiceConfigPage `AiConfigTab` 三处 Select（加 Key/编辑 Key/加模型）加「自定义厂商」选项，除预设 yunwu/siliconflow/glm/gemini 外可填任意厂商编码（小写字母/数字/短横线，如 `deepseek`）+ 自定义 base_url；**后端 0 改动**（provider 早就是 VARCHAR(64)，adapter 按字符串筛选 Key 池）；协议约束一期仅 OpenAI 兼容；同步契约 §13.1/§13.2 + §15 + frontend README；新增 `AiConfigTab.test.tsx` 6 测试；CI 双绿 12m3s+2m19s）。上一个：2026-07-17（**AIGC 工具回归测试评价体系 — 设计阶段**：在 `feature/aigc-evaluation` 独立分支完成 spec + 分阶段实施计划，spec 过三轮独立 agent review、计划过两轮 review 收敛；产出周会文档 + 数据模型关系图（HTML）+ Open Design 简报；一期=千川仿写 + 3 维度 + AI 初评/人工校准 + 手动/版本创建触发，独立 `eval_` 表、不动现有模块，待周会对齐后进入实施）。同日稍早 **PR #28 + PR #29 + PR #24 合并 main**：PR #28 `feat: restore KOL workspace core workflow`（红人工作台旧版八模块还原 + 视觉优化 + 4 个新 migration 049-052，235 文件 +8258/-1888，PM 接手修 4 处项目红线违规后 merge `ba376ce9`）；PR #29 `ci: 翻严 Gate 1+2 + 补 middlewares/core 覆盖率`（merge `0cc760b5`）；PR #24 `chore: ignore docs/walkthrough/`（merge `4952e88c`）。上一个：2026-07-12 PR #25 + PR #26（红人状态重构 + 工作台入口 + UI 一致性 + 补契约测试）。再上一个：2026-07-07 PR #21 + PR #18。再上一个：2026-07-03 PR #20 + PR #19（provider + AI 多服务商切换）。再上一个：2026-07-01 管理端配置页 UX 完善。再上一个：2026-06-30 PR #13 红人工作台 Sprint 18-23。再上一个：2026-06-28 旧架构数据全量迁移）

> **🚧 当前状态**：main 上最新合并 = **PR #35**（2026-07-31，merge `5997b24b`，自定义 AI 厂商支持）+ AIGC 评价体系 v2 实施（`2394a0d6`）+ **PR #28**（2026-07-17，merge `ba376ce9`）+ PR #29（CI Gate 1+2 翻严 + Gate 3 部分）+ PR #24（chore）+ PR #26 + PR #25（2026-07-12）+ PR #21 + PR #18（2026-07-07）+ PR #20 + PR #19（2026-07-03）。Sprint25 功能分支已达到开发侧验收门禁，等待产品经理验收和创建合并请求；不得据此视为已合并或上线。下一步候选：Sprint25 产品验收 / aigc-evaluation 周会结论落地 / 补 services/routers 覆盖率 → 翻严 coverage gate / legacy 迁移工具归档。
...
### M2 Sprint25 — 红人工作台达人档案统一 🟡 可供产品经理验收（2026-08-03）

**开发结果**：工作台人物档案成为七字段唯一编辑入口；人格定位沿用三步页并绑定正式达人编号；当前运营入驻资料、报告、产出、历史和同步使用同一 `kol_id`；历史未绑定会话保留但不自动匹配；定位字段空值自动写、非空默认保留并逐项确认；五项事实只按报告原文补空；旧入口收口，下游上下文兼容。

**技术证据**：后端相关与邻接 841/841、迁移专项 1/1、前端 489/489、类型检查和生产构建通过；迁移 055 在临时空库全链重放并重复执行；真实页面在 1440×900 和 1024×768 共 6 张截图无横向溢出且无“会话_编号”。
