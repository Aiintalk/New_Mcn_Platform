...
2. **批量字幕提取**：多行分享文本 → PostgreSQL 持久化任务 + 条目 → 后台批量执行 → access_code 跨设备查询
...
| 9 | 批量查询 | 客户端轮询 `GET /batch/{job_code}`（5s 间隔） | ASR 任务 5-30 分钟，SSE 不合适 |
...
3. ✅ 批量任务：多行提交 → 后台执行 → access_code 跨设备查询
...
| Step 4 批量字幕提取 | ✅ | POST /batch + _run_batch + 2 GET 查询 + 6 tests + 前端批量 Tab + conftest patch |
...
测试统计：operator_subtitle 25/25 ✅ + admin_subtitle 8/8 ✅。
