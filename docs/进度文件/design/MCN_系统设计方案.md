...
POST   /api/admin/credentials/{id}/enable
POST   /api/admin/credentials/{id}/disable
POST   /api/admin/credentials/{id}/test
```

### 10.7 AI 统计与模型管理（M2 新增）
...
### 10.8 运营直发会话（M2 Sprint1 新增）

```text
POST /api/operator/intake/direct/start
POST /api/operator/intake/direct/chat
POST /api/operator/intake/direct/submit
GET  /api/operator/intake/direct/{session_id}/status
GET  /api/operator/intake/direct/{session_id}/download
```

### 10.9 运营端首页（M2 Sprint2 新增）
...
kols                   红人：基础信息 + 人格档案 + TikHub 字段 + 抖音号 + 签约人员
...
| `kols` | 红人：`name/category/platform/douyin_id/sec_uid` + TikHub 字段(`account_name`/`avatar_url`/`signature`/`follower_count`/`video_count`/`tikhub_raw`) + 人格档案(`persona`/`content_plan`/`style_notes`) + `owner_id`(签约人员，外键→users) + `status/created_by` |
...
| 表名 | 说明 |
|---|---|
| `credentials` | AI Key 池（yunwu等供应商）：`provider`/`label`/`api_key`/`base_url`/`status`/`active_requests`/`max_concurrent`/`max_users`/`last_tested_at`/`last_latency_ms` |
| `ai_models` | 可用 AI 模型配置：`name`/`provider`/`model_id`/`status`/`last_tested_at`/`last_latency_ms` |
| `ai_call_logs` | yunwu AI 调用明细日志：`user_id`/`feature`/`model_id`/`credential_id`/`input_tokens`/`output_tokens`/`latency_ms`/`status`/`error_message` |
...
| `service_credentials` | **第三方服务密钥池**（TikHub/OSS/ASR）：`provider`/`label`/`secret_enc`/`secret_tail`/`status`/`weight`/`quota_limit`/`quota_used`/`fail_count`/`cooldown_until`/`config`；OSS 单组凭证不入池 |
...
- **密钥池（分两套机制）：**
  - **AI Key 池**（表 `credentials`）：yunwu 等供应商的 API Key，支持并发控制（`active_requests`/`max_concurrent`），多 Key 轮询调度
  - **第三方服务密钥池**（表 `service_credentials`）：TikHub/OSS/ASR 等服务，支持权重配额、失败冷却、操作日志记录
- **密钥管理功能**：管理员端「服务配置」页面支持增删改停、连通性测试、调用统计查看
- **失败/超时/重试**：每次外部调用统一 `success/timeout/failed` 三态并写日志；按 key 的重试策略执行，耗尽置 job `failed` 并回显可读错误。
...
- TikHub 集成字段：**抖音号 `douyin_id`**、**TikHub 长标识 `sec_uid`**、**头像 `avatar_url`**、**简介 `signature`**、**粉丝数 `follower_count`**、**视频数 `video_count`**、**原始数据 `tikhub_raw`**
- 新增字段：**签约人员 `owner_id`**（下拉选用户管理在职员工，存员工 id，支持按员工反查名下红人）、**抖音昵称 `account_name`**
- 选达人步骤只展示 `status=active` 的红人。
- 创建红人与 TikHub 抓取解耦：创建时不触发 TikHub 抓取，独立「重新抓取」按钮手动触发。
...
| `kol_intake_links` | 一次性分享链接（博主模式） |
| `kol_intake_submissions` | 对话记录与生成报告（博主模式） |
| `kol_intake_operator_sessions` | 运营直发对话会话（运营模式） |
...
| `operator_intake.py` | `/api/operator/intake` | 运营端接口（分享链接管理） |
| `operator_intake_direct.py` | `/api/operator/intake/direct` | 运营直发接口（不走链接） |
