...
> **状态**：设计稿（待评审）  
> **日期**：2026-07-17  
> **背景会议**：0605 视频制作流程探讨、0709 开发工具与工作流优化  
...
| 维度 | 一期范围 | 未来扩展 |
|------|---------|---------|
| 被测工具 | 千川仿写（qianchuan-writer） | 千川复盘、直播脚本、视频成片、达人表现等 |
| 被测输出 | 脚本文案 | 视频、音频、结构化数据 |
| 测试集来源 | 人工录入 | 从历史 outputs 导入、运行时采集、生产数据 AI 辅助回捞 |
| 工具版本 | 平台内显式创建版本快照 | 与 git commit / CI 集成 |
| 评分方式 | AI 初评 + 人工校准 | 外部评分服务、模型微调 |
| 评分维度 | 文案质量、种草力、人设一致性 | 卖点覆盖、合规安全、结构符合度、信息量等 |
| 调度触发 | 手动触发 + 版本创建/复制后自动触发 + 定时批量 | 与 CI/CD、部署流水线联动 |
| 与现有工作流打通 | 不做打通 | 轻量/深度打通 |
...
| `app/services/workspace_prompt.py` | 评测生成器需复用 `resolve_prompt` 处理红人覆盖 | ❌ 不改，只读调用 |
...
| 对接点 | 当前状态 | 一期要求 | 未来改造点 |
|--------|---------|---------|-----------|
| `qianchuan_writer_configs.system_prompt` | 存在 DB 配置表 | 测试平台手动复制为版本快照 | 提供 API 让测试平台一键导入当前配置 |
...
📄 `docs/evaluation/data-model-diagram.html`

无需安装任何软件，浏览器打开即可看到 9 张表的关系图（mermaid.js 自动渲染）。
...
| `eval_rubrics.scenario_tag` | 一期可为空 | 支持不同场景/达人使用不同 rubric |
...
- `POST /api/operator/evaluation/runs` — 触发运行
...
| 文档 | 路径 | 用途 |
|------|------|------|
| 架构设计 | `docs/superpowers/specs/2026-07-17-aigc-evaluation-system-design.md` | 本文件 |
| 数据模型关系图 | `docs/evaluation/data-model-diagram.html` | 浏览器直接打开查看表关系图 |
