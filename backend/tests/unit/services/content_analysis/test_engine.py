...
async def test_previous_day_without_history_stays_out_of_report_but_can_enter_library() -> None:
    ...
    report = result.reports["project-a"]
    assert report.items == ()
    assert [item.stable_key for item in report.all_window_items] == [
        "platform_content_id:work-previous"
    ]
    assert report.all_window_items[0].is_opportunity is True
    assert report.all_window_items[0].in_library is True
    assert report.all_window_items[0].candidate_rank == 1
    assert report.previous_two_day_changes == ()
    assert report.persona_opportunities == ()
...
@pytest.mark.asyncio
async def test_company_cross_project_pool_keeps_saved_projects_outside_current_task() -> None:
    method = reusable_method("problem-proof", "问题到证明", "先问题后证明")
    ...
    assert len(result.cross_project_candidates) == 1
    assert result.cross_project_candidates[0].project_ids == (
        "project-a",
        "project-out",
    )
    assert result.reports["project-a"].cross_project_candidates == result.cross_project_candidates


@pytest.mark.asyncio
async def test_company_cross_project_pool_keeps_saved_project_without_current_relation() -> None:
    method = reusable_method("problem-proof", "问题到证明", "先问题后证明")
