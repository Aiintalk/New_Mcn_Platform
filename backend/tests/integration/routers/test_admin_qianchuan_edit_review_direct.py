"""
Direct-call coverage tests for admin_qianchuan_edit_review.py。
...
from app.routers.admin_qianchuan_edit_review import ConfigIn, list_configs, update_config


@pytest_asyncio.fixture(autouse=True)
async def _cleanup(test_session):
    """清 qianchuan_edit_review_configs 防污染。"""
    await test_session.execute(delete(QianchuanEditReviewConfig))
...
async def _seed(test_session, key="default", **kw) -> QianchuanEditReviewConfig:
    cfg = QianchuanEditReviewConfig(
        config_key=key, system_prompt=kw.get("system_prompt", "p"), is_active=True,
    )
...
async def test_list_configs(test_session, admin_user):
    """空表 + 有行 → 都不报错。"""
    await _seed(test_session, key="a")
    await _seed(test_session, key="b")
...
async def test_update_config_ok(test_session, admin_user):
    """配置存在 → update+returning 走通。"""
    cfg = await _seed(test_session, key="default")
