...
- 最终聚焦测试 84 通过，聚焦覆盖率 97%；最新完整门禁 2097 通过、1 跳过、0 失败、0 错误，整体覆盖率 79.9%，六个分层全部通过。
- 3 条静态 fixture（测试固定输入）只是匿名合成内容样本，仅用于字段/隐私形态检查；不含项目关系，不经适配层驱动引擎。多项目关系由 `test_engine.py`（离线引擎单元测试）中的独立匿名合成对象验证。
