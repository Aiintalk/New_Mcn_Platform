...
> 代码对象：`9c4897ae6749350b1a5450cae5d926dbcbe5bc86`（本轮最终代码收口提交）

## 一、结论

最终内容分析聚焦测试 84/84 通过，聚焦覆盖率 97%。最新完整覆盖率门禁在 `backend`（后端目录）执行，收集 2098 条，2097 通过、0 失败、0 错误、1 跳过、14 条警告，退出码为 0。开发侧离线切片可交主产品经理独立审核。
...
- 最终结果：84 通过，0 失败，耗时 0.22 秒，退出码 0。
- 根会话最新专项覆盖率证据：97%。
...
|---:|---:|---:|---:|---:|---:|---:|---:|
| 2098 | 2097 | 0 | 0 | 1 | 14 | 420.17 秒 | 0 |

### 4. 中间环境失败与定位（非最终验证）

- 最终代码的一次完整门禁在共享测试库会话异常时出现 1118 通过、47 失败、933 错误、1 跳过。
- 未改动业务代码，使用全新 pytest（Python 测试进程）重跑上次缓存的 978 个失败/错误项，结果 978 通过、0 失败、867 未选中，耗时 321.71 秒。
- 随后按第 3 节原命令重跑，2097 通过、1 跳过、0 失败、0 错误，六层门禁均通过。因此本轮未修改内容分析以外的测试基础设施，但保留共享测试库状态对长回归的稳定性风险。
...
- 直接执行 `pytest tests/ --collect-only`（尝试收集所有测试）收集到 2106 项后以退出码 2 停止；原因是既有 `tests/intake/conftest.py` 在非顶层声明 `pytest_plugins`（pytest 插件配置），新版 pytest 拒绝该布局。本轮没有修改测试基础设施；仓库正式覆盖率脚本明确运行 `tests/unit/` 与 `tests/integration/`，最终门禁已干净通过。收集插件曾改写 M1 并发测试报告的日期，已只恢复该测试副作用，未纳入提交。
...
最大回归和覆盖率测试生成了 3 个未跟踪 Word 文件：`questionnaire_template.docx`（问卷模板）、`11_profile.docx`（人格档案报告）、`11_plan.docx`（内容规划报告）。核对时间、路径和测试引用后，已只将这 3 个测试产物移入系统废纸篓的本轮专用目录，仍可恢复，未处理其他文件。
