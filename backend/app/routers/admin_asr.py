"""
app/routers/admin_asr.py

ASR（阿里云智能语音交互）调用统计接口（admin）：
  GET /api/admin/asr/stats       — 三维统计（overview + operations + users + trend）
  GET /api/admin/asr/operations  — 按 operation（submit/query）聚合
  GET /api/admin/asr/users       — 按用户聚合调用排行

参照 admin_oss.py 实现，区别：
- 表名：oss_call_logs → asr_call_logs
- operation 取值：submit/query（vs OSS 的 upload/download/delete）
- 凭证池用通用 service_credentials（WHERE provider='asr'）
"""
...
# ---------------------------------------------------------------------------
# 1. GET /api/admin/asr/stats — 三维统计
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 2. GET /api/admin/asr/operations — 按 operation 聚合
# ---------------------------------------------------------------------------

@router.get("/operations", response_model=ApiResponse)
async def list_asr_operations(
    current_user: User = Depends(require_admin),
    db: AsyncSession = Depends(get_db),
):
...
# ---------------------------------------------------------------------------
# 3. GET /api/admin/asr/users — 用户调用排行
# ---------------------------------------------------------------------------

@router.get("/users", response_model=ApiResponse)
async def list_asr_users(
    start_date: str = "",
    end_date: str = "",
    limit: int = 20,
    current_user: User = Depends(require_admin),
