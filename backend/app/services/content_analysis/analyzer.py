...
class ProjectAssessment:
    ...
    cross_project_signals: tuple[CrossProjectSignal, ...] = ()
    cross_project_scenarios: tuple[SourceConstraint, ...] = ()
    ...
    def __post_init__(self) -> None:
        ...
        if any(
            not isinstance(signal, CrossProjectSignal)
            for signal in self.cross_project_signals
        ):
            raise ValueError("跨项目信号必须使用封闭枚举")
        if any(
            not isinstance(item, SourceConstraint)
            for item in self.cross_project_scenarios
        ):
            raise ValueError("跨项目适用场景必须使用结构化边界")
        if any(
...
def enforce_project_assessment_boundaries(
    ...
    _validate_generated_claims(
        (
            ...
            *(item.statement for item in assessment.cross_project_scenarios),
    ...
    _validate_boundary_claims(
        ...
        + tuple(item.statement for item in assessment.cross_project_scenarios),
        visual_is_readable=visual_is_readable,
    )
    if any(
        value
