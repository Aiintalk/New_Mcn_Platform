// frontend/src/pages/admin/LivestreamReviewConfigTab.tsx
...
interface LivestreamReviewConfig {
  id: number;
  config_key: string;
...
const CONFIG_LABELS: Record<string, string> = {
  with_excel:    '有直播数据版（hasExcel = true）',
  without_excel: '仅脚本版（hasExcel = false）',
};

export default function LivestreamReviewConfigTab() {
  const { message } = App.useApp();
  const [configs, setConfigs] = useState<LivestreamReviewConfig[]>([]);
  const [models, setModels] = useState<AiModelItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [editingConfig, setEditingConfig] = useState<LivestreamReviewConfig | null>(null);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [cfgResp, mdResp] = await Promise.all([
        get<LivestreamReviewConfig[]>('/api/admin/livestream-review/configs'),
  ...
  function openEdit(cfg: LivestreamReviewConfig) {
  ...
  async function saveConfig(values: { ai_model_id: number | null; system_prompt: string | null }) {
    ...
    try {
      await put(`/api/admin/livestream-review/configs/${editingConfig.config_key}`, {
      ...
      >
        <Form form={configForm} layout="vertical" onFinish={saveConfig} style={{ marginTop: 16 }}>
          <Form.Item label="AI 模型" name="ai_model_id">
            <Select
              placeholder="选择已配置的 AI 模型（留空使用默认 claude-sonnet-4-20250514）"
