...
import type { QianchuanPreviewConfig } from '../../types/qianchuanPreview';
...
export default function QianchuanPreviewConfigTab() {
  ...
  const [configs, setConfigs] = useState<QianchuanPreviewConfig[]>([]);
  const [models, setModels] = useState<AiModelItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [editingConfig, setEditingConfig] = useState<QianchuanPreviewConfig | null>(null);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [cfgResp, mdResp] = await Promise.all([
        get<QianchuanPreviewConfig[]>('/api/admin/qianchuan-preview/configs'),
  ...
  function openEdit(cfg: QianchuanPreviewConfig) {
  ...
  async function saveConfig(values: { ai_model_id: number | null; system_prompt: string | null }) {
    ...
    try {
      await put(`/api/admin/qianchuan-preview/configs/${editingConfig.config_key}`, {
