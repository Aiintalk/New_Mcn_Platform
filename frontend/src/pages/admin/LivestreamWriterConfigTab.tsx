// frontend/src/pages/admin/LivestreamWriterConfigTab.tsx
...
interface LivestreamWriterConfig {
  id: number;
  config_key: string;
...
export default function LivestreamWriterConfigTab() {
  const { message } = App.useApp();
  const [configs, setConfigs] = useState<LivestreamWriterConfig[]>([]);
  const [models, setModels] = useState<AiModelItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [editingConfig, setEditingConfig] = useState<LivestreamWriterConfig | null>(null);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [cfgResp, mdResp] = await Promise.all([
        get<LivestreamWriterConfig[]>('/api/admin/livestream-writer/configs'),
  ...
  function openEdit(cfg: LivestreamWriterConfig) {
  ...
  async function saveConfig(values: { ai_model_id: number | null; system_prompt: string | null }) {
    ...
    try {
      await put(`/api/admin/livestream-writer/configs/${editingConfig.config_key}`, {
      ...
      >
        <Form form={configForm} layout="vertical" onFinish={saveConfig} style={{ marginTop: 16 }}>
          ...
          <Form.Item label="系统 Prompt" name="system_prompt">
            <Input.TextArea rows={16} placeholder="输入系统 Prompt..." style={{ fontFamily: 'var(--font-mono)', fontSize: 12 }} />
          </Form.Item>
        </Form>
      </Modal>
    </>
