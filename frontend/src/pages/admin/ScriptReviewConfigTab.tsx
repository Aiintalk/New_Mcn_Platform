...
import { getConfig, updateConfig } from '../../api/scriptReview';
import { getAiModels } from '../../api/ai';
import type { ScriptReviewConfig } from '../../types/scriptReview';
...
export default function ScriptReviewConfigTab() {
  const { message } = App.useApp();
  const [config, setConfig] = useState<ScriptReviewConfig | null>(null);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      form.setFieldsValue({
        direct_prompt: cfg.direct_prompt,
        value_prompt: cfg.value_prompt,
  ...
  async function handleSave(values: FormValues) {
    ...
    try {
      await updateConfig({
        direct_prompt: values.direct_prompt ?? null,
        value_prompt: values.value_prompt ?? null,
    ...
    <div style={{ maxWidth: 860 }}>
      <div className="card" style={{ marginBottom: 0 }}>
        <div className="card-header">
          <h2 className="card-title">千川脚本预审配置</h2>
        ...
        <div className="card-body">
          <Form form={form} layout="vertical" onFinish={handleSave}>
            <Form.Item label="千川直销模式 Prompt" name="direct_prompt">
              <TextArea
                rows={10}
                placeholder="输入千川直销模式审核 Prompt..."
                style={{ fontFamily: 'var(--font-mono)', fontSize: 12 }}
              />
            </Form.Item>
            <Form.Item label="价值观模式 Prompt" name="value_prompt">
              <TextArea
                rows={10}
                placeholder="输入价值观内容模式审核 Prompt..."
                style={{ fontFamily: 'var(--font-mono)', fontSize: 12 }}
              />
            </Form.Item>
