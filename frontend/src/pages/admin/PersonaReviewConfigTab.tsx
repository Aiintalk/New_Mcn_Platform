// frontend/src/pages/admin/PersonaReviewConfigTab.tsx
...
const CONFIG_LABELS: Record<string, string> = {
  with_excel:    '有运营数据版（hasExcel = true）',
  without_excel: '仅脚本版（hasExcel = false）',
};

export default function PersonaReviewConfigTab() {
  const { message } = App.useApp();
  const [configs, setConfigs] = useState<PersonaReviewConfig[]>([]);
  const [models, setModels] = useState<AiModelItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [editingConfig, setEditingConfig] = useState<PersonaReviewConfig | null>(null);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [cfgResp, mdResp] = await Promise.all([
        get<PersonaReviewConfig[]>('/api/admin/persona-review/configs'),
  ...
  function openEdit(cfg: PersonaReviewConfig) {
  ...
  async function saveConfig(values: { ai_model_id: number | null; system_prompt: string | null }) {
    ...
    try {
      await put(`/api/admin/persona-review/configs/${editingConfig.config_key}`, {
