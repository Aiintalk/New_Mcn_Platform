...
- “补齐空字段”只发送一次 `POST /api/operator/kols/{kol_id}/persona-details/fill-empty`，根据响应中的 `filled_fields` 与 `preserved_fields` 显示结果，再重新读取统一档案。
...
- 七个字段均支持编辑、保存、取消和清空；同一时间只允许编辑一个字段，保存时只提交当前字段。
- “补齐空字段”不再逐字段确认；页面直接显示哪些字段已补齐、哪些字段因已有内容而保留。
- 管理端达人新建和编辑表单不再展示或提交人格档案、内容规划；详情页改为只读摘要，并提供“前往红人工作台编辑”。
- 旧素材库删除 `updateKolProfile` 导出及页面中的档案保存、AI 生成人格入口；只读摘要与工作台跳转替代编辑器。六种参考资料类型及新增、删除行为保持不变。
...
- 本提交不包含独立审查结论。独立审查由父任务在提交后执行，若发现问题应形成新的受控修复闭环。
